...
local function SetupBackpackView(frameGroup)
	local allBackpackViews = {
		single = CreateFrame(
			"Frame",
			"Baganator_SingleViewBackpackViewFrame" .. frameGroup,
			UIParent,
			"BaganatorSingleViewBackpackViewTemplate"
		),
		category = CreateFrame(
			"Frame",
			"Baganator_CategoryViewBackpackViewFrame" .. frameGroup,
			UIParent,
			"BaganatorCategoryViewBackpackViewTemplate"
		),
	}
...
local function SetupBankView(frameGroup)
	local allBankViews = {
		single = CreateFrame(
			"Frame",
			"Baganator_SingleViewBankViewFrame" .. frameGroup,
			UIParent,
			"BaganatorSingleViewBankViewTemplate"
		),
		category = CreateFrame(
			"Frame",
			"Baganator_CategoryViewBankViewFrame" .. frameGroup,
			UIParent,
			"BaganatorCategoryViewBankViewTemplate"
		),
	}
	...
	addonTable.CallbackRegistry:RegisterCallback("BankToggle", function(_, entity, subView)
		...
		elseif selectedTab == addonTable.Constants.BankTabType.Warband then -- Warband bank
			...
			bankView:SetShown(
				addonTable.Config.Get(addonTable.Config.Options.BANK_CURRENT_TAB)
						~= addonTable.Constants.BankTabType.Warband
					or not bankView:IsShown()
			)
...
local function SetupGuildView(frameGroup)
	local guildView = CreateFrame(
		"Frame",
		"Baganator_SingleViewGuildViewFrame" .. frameGroup,
		UIParent,
		"BaganatorSingleViewGuildViewTemplate"
	)
	...
	addonTable.CallbackRegistry:RegisterCallback("GuildToggle", function(_, guildName)
		...
		guildView:UpdateForGuild(
			guildName,
			Syndicator.API.GetCurrentGuild() == guildName
	...
	addonTable.CallbackRegistry:RegisterCallback("GuildShow", function(_, guildName, tabIndex)
		...
		guildView:UpdateForGuild(
			guildName,
			guildName == Syndicator.API.GetCurrentGuild()
				and C_PlayerInteractionManager.IsInteractingWithNpcOfType(Enum.PlayerInteractionType.GuildBanker)
		)
	end)
...
local function SetupCharacterSelect(frameGroup)
	local characterSelect = CreateFrame(
		"Frame",
		"Baganator_CharacterSelectFrame" .. frameGroup,
		UIParent,
		"BaganatorCharacterSelectTemplate"
	)
